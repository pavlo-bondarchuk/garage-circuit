...
const original=c;c={...c,id:c.visualBase||c.id};const signature=true;const base=designs[c.id]||designs['301'];const longTruck=c.id==='kenworth';const scale=original.bodyLength&&!longTruck?original.bodyLength/base.l:1;const d={...base,l:longTruck?base.l:original.bodyLength||base.l,w:longTruck?base.w:original.bodyWidth||base.w,roof:longTruck?base.roof:original.bodyHeight||base.roof,axles:base.axles.map(z=>z*scale),front:base.front*scale,rear:base.rear*scale,rf:base.rf*scale,rr:base.rr*scale};const g=new T.Group();g.name=c.name;g.userData={wheels:[],parts:[],modelVersion:signature?5:4,bodyType:c.type};
